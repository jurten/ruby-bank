class BranchesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @branch.save
        format.html { redirect_to branch_url(@branch), notice: "La Sucursal fue creada con exito" }
  ...
  def update
    respond_to do |format|
      if @branch.update(branch_params)
        format.html { redirect_to branch_url(@branch), notice: "La Sucursal fue actualizada con exito" }
  ...
  def destroy
    if @branch.turns.where(status: :pending).empty?
      ...
      redirect_to branches_url, notice: "No se puede borrar una sucursal con turnos pendientes"
    end
  end
